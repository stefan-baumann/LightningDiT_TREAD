...
class LightningDiTBlock(nn.Module):
    ...
    @torch.compile
    def forward(self, x, c, feat_rope=None, rope_ids: Optional[torch.Tensor] = None):
        if self.wo_shift:
...
class LightningDiT(nn.Module):
    ...
    def forward(self, x, t=None, y=None):
        ...
        for idx, block in enumerate(self.blocks):
            ...
            if do_route and ids_keep_active is None and idx == route_start_idx:
            ...
            # Apply RoPE everywhere.
            # When routed, we also pass the original (flattened) indices 'ids_keep_active' so RoPE rotates q,k
            # using the *correct* positions despite the arbitrary order of the routed subsequence.
            if use_checkpoint:
                x = checkpoint(
                    block, x, c, self.feat_rope, ids_keep_active, use_reentrant=True
                )
            else:
                x = block(x, c, self.feat_rope, ids_keep_active)

            # end routing: scatter tokens back to original positions
            if do_route and ids_keep_active is not None and idx == route_end_idx:
